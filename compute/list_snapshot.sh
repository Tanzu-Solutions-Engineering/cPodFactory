...
echo "Listing snapshot for VMs of '${HEADER}-${1}'."
docker run --rm --dns=${DNS} --entrypoint="/usr/bin/pwsh" -it -v /tmp/scripts:/tmp/scripts vmware/powerclicore:12.4 ${SCRIPT} 
echo "VM" | grep 'VM|Name|Size|Children'
