...
CLUSTERIMAGEID=$(sddc_get_personalities_full "${NAME_LOWER}" "${TOKEN}" |jq -r .elements[].personalityId)
...
echo
VALIDATIONSTATUS=$(sddc_post_domain_validation "${NAME_LOWER}" "${TOKEN}" "${DOMAINJSON}" )
...
echo
CREATIONSTATUS=$(sddc_post_domain_creation "${NAME_LOWER}" "${TOKEN}" "${DOMAINJSON}" )
echo "${CREATIONSTATUS}"
echo
CREATIONID=$(echo "${CREATIONSTATUS}" |jq -r '.id')

echo
sddc_loop_wait_commissioning  "${CREATIONID}"
