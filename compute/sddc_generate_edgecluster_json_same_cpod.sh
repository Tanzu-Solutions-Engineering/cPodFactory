...
[ "$1" == "" ] || [ "$2" == "" ] || [ "$3" == "" ] && echo "usage: $0 <name_of_vcf_cpod> wldname cluster "  && echo "usage example: $0 vcf45 wld01 wld01-cl01" && exit 1
...
DOMAINID=$(sddc_domains_id_get "${WLDNAME}") 
if [ "${DOMAINID}" == "" ]
then
        echo "No domain ID found for ${WLDNAME}"
        echo "bailing out"
        exit 1
else
        echo "Domain ID found for ${WLDNAME} : ${DOMAINID}"
fi


# Check Clusters
CLUSTERID=$(sddc_cluster_id_get "${CLUSTERNAME}") 
if [ "${CLUSTERID}" == "" ]
then
        echo "No Cluster ID found for ${CLUSTERNAME}"
        echo "bailing out"
        exit 1
else
        echo "Cluster ID found for ${CLUSTERNAME} : ${CLUSTERID}"
fi
