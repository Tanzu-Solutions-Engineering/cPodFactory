...
then
        ...
        create_t0_interface_v2 "${T0GWNAME}" "${EDGECLUSTERID}" "${T0IP03}" "${T0AZ3SEGMENTNAME}" "${EDGEIDX03}" "edge-az3-uplink-1" "${LOCALESERVICE}"
...
then
        ...
        configure_tier-0s_bgp_neighbor_v2 "${T0GWNAME}"  "${AZ1CPODASNIP}"  "${AZ1ASNCPOD}"  "${AZ1CPOD_NAME_LOWER}" "${LOCALESERVICE}" "${T0IP01}" 
        #AZ2
