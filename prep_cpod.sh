...
for ((i=1; i<=NUM_ESX; i++)); do
  ...
  echo "$ESXHOST --- Setting ntp to: $TRANSIT_IP  ---"
	...
	sshpass -p "${ROOT_PASSWD}" scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=error ./install/ssd_esx_tag.sh root@"${DHCPIP}":/root/ssd_esx_tag.sh
	sshpass -p "${ROOT_PASSWD}" ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=error root@"${DHCPIP}" "/root/ssd_esx_tag.sh"
